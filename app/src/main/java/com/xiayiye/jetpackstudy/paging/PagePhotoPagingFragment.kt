...
import com.xiayiye.jetpackstudy.gallery.PagerPhotoListAdapter
...
class PagePhotoPagingFragment : Fragment() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        val pagerPhotoListAdapter = PagerPhotoListAdapter()
        pagerPhotoListAdapter.apply {
            vp2Banner.adapter = this
        }
        galleryViewModelPaging.pageListLiveData.observe(viewLifecycleOwner, Observer {
        ...
        vp2Banner.registerOnPageChangeCallback(object : ViewPager2.OnPageChangeCallback() {
            override fun onPageSelected(position: Int) {
                ...
                tvShowImagePage.text =
                    StringBuffer().append(position + 1).append("/")
                        .append(galleryViewModelPaging.pageListLiveData.value?.size)
            }
        })
        //设置纵向滚动的方法
